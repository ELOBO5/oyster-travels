        ...
        const getReactionInfo = async () => {
            try {
                const { data } = await axios.get(
                    `https://oystertravel.herokuapp.com/api/experiences/${experience.id}/`
                );

                const currentUserId = Number(localStorage.getItem('user_id'));

                const userHasLiked = data.reactions.find(
                    reaction => reaction.like && reaction.user === currentUserId
                );

                const userHasDisliked = data.reactions.find(
                    reaction => reaction.dislike && reaction.user === currentUserId
                );
